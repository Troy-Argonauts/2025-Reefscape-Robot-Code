...
public class RobotContainer {
  ...
  private void configureBindings() {
	...
	Robot.getDrivetrain().setDefaultCommand(
	  new RunCommand(
		() -> {
			double xSpeed = (Math.abs(driver.getLeftX()) > Constants.Controllers.DEADBAND) ? driver.getLeftX(): 0; 
			double ySpeed = (Math.abs(driver.getLeftY()) > Constants.Controllers.DEADBAND) ? driver.getLeftY(): 0;
			double rotSpeed = (Math.abs(driver.getRightX()) > Constants.Controllers.DEADBAND) ? driver.getRightX(): 0;
	...
	driver.x().whileTrue(
		new InstantCommand(() -> Robot.getDrivetrain().setXState(true))
	).whileFalse(
